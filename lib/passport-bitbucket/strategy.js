...
function Strategy(options, verify) {
  ...
  this._userEmailsURL = options.userEmailsURL || 'https://api.bitbucket.org/2.0/user/emails';
...
Strategy.prototype.userProfile = function(accessToken, done) {
  this._oauth2.get(this._userProfileURL, accessToken, (err, body, res) => {
    ...
    this._oauth2.get(this._userEmailsURL, accessToken, (err, body, res) => {
      if (err) {
        return done(new InternalOAuthError('Failed to fetch user profile', err));
      }
      ...
      profile.emails = Emails.parse(json);
